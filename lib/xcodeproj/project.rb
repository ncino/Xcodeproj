...
module Xcodeproj
  ...
  class Project
    ...
    # @return [String] an optional string containing contents of an Xcode project file
    #
    attr_reader :project_string

    # @param  [Pathname, String] path @see path
    #         The path provided will be expanded to an absolute path.
    # @param  [String] string_contents @see project_string
    #         The full contents of an Xcode project file in string format.
    ...
    #       manually set the object version. Additionally, you can either specify
    #       a path to an Xcode project file (path) or a string containing the contents
    #       of an Xcode project (string_contents). You must at least include one of
    #       those options.
    #
    def initialize(path, string_contents = nil, skip_initialization = false, object_version = Constants::DEFAULT_OBJECT_VERSION)
      unless !path.nil? || !string_contents.nil?
        raise ArgumentError, '[Xcodeproj] Initialization parameter expected to include ' \
          'a `path` or `string_contents` value.'
      end
      if !path.nil?
        @path = Pathname.new(path).expand_path
        @project_dir = @path.dirname
      else
        @project_string = string_contents
      end
      @objects_by_uuid = {}
      @generated_uuids = []
      @available_uuids = []
    ...
    # Opens a project contained in the string.
    #
    # @param  [String] contents
    #         The Xcode project in string format.
    #
    # @raise  If the project versions are more recent than the ones know to
    #         Xcodeproj to prevent it from corrupting existing projects.
    #         Naturally, this would never happen with a project generated by
    ...
    #         malformed.
    #
    # @example Opening a project from a string
    #         Project.read(contents_of_a_project_in_a_string)
    #
    def self.read(contents)
      unless !contents.empty?
        raise "[Xcodeproj] Unable to read an empty project string."
      end
      project = new(nil, contents, true)
      project.send(:initialize_from_string)
      project
    end
    ...
    def initialize_from_file
      ...
      initialize_from_plist(plist)
    end

    # Initializes the instance with the project inside the `project_string`
    # atttribute, which is a full string of an Xcode project.
    #
    def initialize_from_string
      plist = Plist.read_from_string(project_string)
      initialize_from_plist(plist)
    end

    # Helper initializer to complete the initialization from an existing
    # project file or project string.
    #
    # @param [Plist] plist
    #        The plist object loaded from a file or string.
    #
    def initialize_from_plist(plist)
      root_object.remove_referrer(self) if root_object
